defmodule HayaiLedgerWeb.Plugs.Context do
  ...
  def call(conn, _) do
    with %{ organization_id: _ } = context <- build_context(conn) do
  ...
  def build_context(conn) do
  	with [key_data] when is_binary(key_data) <- get_req_header(conn, "x-api-key"),
			...
			{:ok, {api_type, organization_id}} <- verify_token(token_salt, key_data),
			true <- organization_id == org_id,
			Absinthe.Plug.put_options(conn, api_type: api_type)
		do
